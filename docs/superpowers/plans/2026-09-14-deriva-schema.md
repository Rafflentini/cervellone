...
2. **Il controllo positivo sul campo**, che è la prova che tutto questo serve a qualcosa: al primo giro vero il guardiano **deve ritrovare 2 delle 3 migrazioni inerti già note** — `2026-05-07-gmail-classification` (tabella `cervellone_gmail_categorie`, indice `idx_gmail_categorie_enabled`, chiave `gmail_classify_last_run`) e `2026-05-09-v19-foundation` (`agent_runs`, `sub_agent_jobs`, `document_renders`, `e2b_sandboxes`, più le colonne dei loro corpi). **Se dice «nessuna deriva», è rotto** — perché la deriva oggi c'è, e la conosciamo nome per nome.

   **La terza non è coperta, ed è misurato**: `2026-05-09-v19-memories-bucket.sql` produce **zero** oggetti attesi. Contiene solo un `insert into storage.buckets` e un `create policy` dentro un `do $$ … $$` — forme che il controllo **dichiara di non leggere**, e infatti quel file compare con 2 statement fra i «non interpretati». Non è una perdita silenziosa: è il secondo numero del rapporto che fa il suo mestiere. Non si allarga il parser a `storage.*` in questo giro.
