# Contabilità Sub-progetto A (Fatture in Cloud read-only) — Implementation Plan

> **For agentic workers:** eseguito da Codex (un task `.loop/queue/NNN-*.md` per unità) con review/merge/deploy di Claude; se Codex non è disponibile lo fa Claude. Sandbox offline: niente build/test locali. Verifica = review diff + `next build` su Vercel (READY) + smoke. Step con checkbox `- [ ]`.

**Goal:** dare a Cervellone la lettura di Fatture in Cloud (fatture emesse/ricevute, anagrafiche) in sola lettura, come base dell'Amministrazione Contabile.

**Architecture:** un client `fatture-in-cloud.ts` che espone SOLO `ficGet` (HTTP GET) — la scrittura è impossibile per costruzione. Sopra il client, 4 tool di lettura registrati nel tool registry. Token solo in env Vercel.

**Tech Stack:** TypeScript, `fetch` nativo (Node 24), Anthropic tool schema, Fatture in Cloud API v2 (`https://api-v2.fattureincloud.it`).

---

## File structure
- **Create** `src/lib/fatture-in-cloud.ts` — client `ficGet` + `getCompanyId` + `FIC_READ_TOOLS` + `executeFicTool`. Unica responsabilità: lettura FIC.
- **Modify** `src/lib/tools.ts` — registra i tool.
- **Modify** `src/lib/prompts.ts` — regola "Amministrazione contabile".
- **Pre-flight (utente):** aggiungere env `FIC_ACCESS_TOKEN` su Vercel (Production). `FIC_COMPANY_ID` opzionale.

Mappa task Codex: A1=047, A2=048, A3=049.

---

## Pre-flight (utente, non codice)
- [ ] Aggiungere su Vercel (progetto `cervellone`, env Production) la variabile `FIC_ACCESS_TOKEN` = token personale FIC (dalla chat Cowork). Opzionale `FIC_COMPANY_ID` (altrimenti auto-risolto). Redeploy dopo l'aggiunta.

---

## Task 047 (A1): client + tool `fatture-in-cloud.ts`

**Files:** Create `src/lib/fatture-in-cloud.ts`

- [ ] **Step 1: client read-only + tool** — implementazione completa:
...
function mapDoc(d: any) {
  return {
    ...
    pagata: d?.is_marked ?? (Array.isArray(d?.payments_list) ? d.payments_list.every((p: any) => p?.status === 'paid') : null),
...
export const FIC_READ_TOOLS: ToolDefinition[] = [
  {
    ...
    input_schema: {
      type: 'object',
      properties: {
        anno: { type: 'integer' }, mese: { type: 'integer' },
        cliente: { type: 'string' },
  ...
  },
  {
    ...
    description: 'Dettaglio completo di un documento FIC dato il suo id e il tipo ("emessa"|"ricevuta").',
    input_schema: {
      type: 'object',
...
- [ ] **Step 2: commit** — `feat(contabilita): client Fatture in Cloud read-only + 4 tool lettura`
- [ ] **Verifica:** review Claude (conferma: nessuna funzione di scrittura nel modulo; ficGet è l'unico accesso rete) + `next build` Vercel. NB i nomi-campo FIC (amount_gross, payments_list, next_due_date) vanno **verificati su una risposta reale allo smoke**: il mapping usa `?.` difensivo, ma se un campo è nominato diversamente si aggiusta `mapDoc` (no crash, solo valori null).

## Task 048 (A2): registrazione tool

**Files:** Modify `src/lib/tools.ts`

- [ ] **Step 1:** import `import { FIC_READ_TOOLS, executeFicTool } from './fatture-in-cloud'`.
- [ ] **Step 2:** in `ALL_TOOLS` aggiungi `...FIC_READ_TOOLS, // 2026-05-26 Contabilità A: Fatture in Cloud read-only`.
- [ ] **Step 3:** in `EXECUTORS` aggiungi `executeFicTool`.
- [ ] **Step 4: commit** — `feat(contabilita): registra i tool Fatture in Cloud nel registry`
- [ ] **Verifica:** review + build Vercel.

## Task 049 (A3): regola prompt

**Files:** Modify `src/lib/prompts.ts`

- [ ] **Step 1:** aggiungere una "REGOLA AMMINISTRAZIONE CONTABILE": Fatture in Cloud è la fonte ufficiale dei dati contabili; in questa fase Cervellone **legge e propone, NON scrive** sul gestionale (i tool fic_* sono di sola lettura). Per importi/scadenze citare sempre numero e data fattura. Se un tool risponde "FIC_ACCESS_TOKEN non configurato", dire all'utente di aggiungerlo su Vercel. NIENTE backtick markdown nel template literal (lezione e61f3b9: usa apostrofi).
- [ ] **Step 2: commit** — `feat(contabilita): regola prompt amministrazione contabile (read-only)`
- [ ] **Verifica:** review + build Vercel + smoke (dopo che l'utente ha messo il token): "quali fatture ho emesso questo mese?", "fatture ricevute da registrare", "cerca cliente X".

---

## Self-review (coverage spec → task)
- Client read-only (solo ficGet, no write) → 047 (modulo senza funzioni di scrittura) ✓
- getCompanyId + env FIC_COMPANY_ID opzionale + cache → 047 ✓
- 4 tool lettura (emesse/ricevute/dettaglio/anagrafica) → 047 ✓
- Gestione token mancante/401/429 → 047 (ficGet) ✓
- Audit console.log → 047 ✓
- Registrazione tool → 048 ✓
- Regola prompt read-only → 049 ✓
- Env FIC_ACCESS_TOKEN → pre-flight utente ✓
- Non-goal (no write/riconciliazione/prima nota) → rispettati ✓
