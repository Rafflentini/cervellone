...
const DICE_COLLEGATA = 'devono esserci i «dati fattura collegata» (DatiFattureCollegate) con NUMERO e DATA della fattura estera'

/** REGOLA 2 — il legame STRUTTURATO con la fattura estera integrata. */
export function regolaFatturaCollegata(doc: Record<string, unknown>): EsitoControllo {
  ...
  if (num && data) {
    return aNorma('fattura_collegata', DICE_COLLEGATA, `IdDocumento ${num}, Data ${data}`)
  }
  ...
  return rilievo(
    ...
    DICE_COLLEGATA,
    !num && !data
      ? '(nessun DatiFattureCollegate)'
      : `riferimento a META\': IdDocumento «${num || '(vuoto)'}», Data «${data || '(vuoto)'}»`,
    'importante',
    'il legame con la fattura estera viaggia solo come testo nella riga e nelle note, cioe\' come una cosa che un umano '
    + 'legge e una macchina no: l\'XML non dice quale documento si sta integrando',
    'si corregge con modifica_documento_fic, campo fattura_collegata (vuole numero e data INSIEME)',
    {
      tool: 'modifica_documento_fic',
      parametri: { id: id ?? 0, fattura_collegata: { numero: '<numero della fattura del fornitore estero>', data: '<data della fattura estera, YYYY-MM-DD>' } },
      // 🚨 Numero e data della fattura estera NON li invento io. Stanno sul PDF
      // del fornitore, e un riferimento sbagliato e' peggio di nessun
      // riferimento, perche' sembra compilato.
      serve_da_te: 'numero e data della fattura ORIGINALE del fornitore estero: leggile dal suo PDF, io non le deduco dalle note',
    },
  )
}
