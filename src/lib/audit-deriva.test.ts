...
beforeEach(() => { derivaPerIlRapporto.mockReset() })
...
describe('self-audit — la sezione sulla deriva', () => {
  it('🚨 c-e ANCHE quando non c-e nessuna deriva', async () => {
    ...
    derivaPerIlRapporto.mockResolvedValue('Nessuna deriva: 40 oggetti del repo sono presenti nel database.\nStatement non interpretati dal controllo: 12.')
  ...
  it('riporta la deriva quando c-e', async () => {
    derivaPerIlRapporto.mockResolvedValue('DERIVA: 3 oggetti su 40 promessi dal repo NON esistono nel database.')
  ...
  it('se il controllo esplode, il rapporto lo DICE e non salta la sezione', async () => {
    derivaPerIlRapporto.mockRejectedValue(new Error('rete giu'))
