...
describe('REGOLA 2 — i dati fattura collegata NON devono esserci', () => {
  // 🚨 Questa regola e' nata al contrario. Il blocco `DatiFattureCollegate`
  // compare nell'XML di una TD17 valida, quindi sembrava che dovesse esserci —
  // ma passato via API dentro `ei_raw`, Fatture in Cloud lo serializza come
  // attributi SCIOLTI `2.1.6.x` e produce blocchi senza `IdDocumento`: la
  // Verifica formale fallisce e lo SdI scarta il documento. Ed e' irreparabile
  // dall'interfaccia. Quindi la PRESENZA e' il difetto.

  it('assenti: a norma — il riferimento sta nella riga e nelle note', () => {
    ...
    expect(c.esito).toBe('a_norma')
  })

  it('🚨 presenti: rilievo GRAVE, e la cura NON e una correzione', () => {
    // ⚠️ La versione precedente di questa regola proponeva di RIMETTERE il
    // blocco con `modifica_documento_fic`: cioe' di rompere l'XML di un
    // documento sano. Una verifica che guasta quello che controlla e' il
    ...
    expect(c.correzione ?? undefined).toBeUndefined()
    expect(c.come).toContain('CANCELLATO')
  })

  it('anche un blocco a META e un rilievo: e comunque XML rotto', () => {
    const c = regolaFatturaCollegata(autofattura({
      ei_raw: {
        FatturaElettronicaBody: {
          DatiGenerali: {
    ...
    expect(c.esito).toBe('rilievo')
    expect(c.trovato).toContain('(vuoto)')
  })
...
describe('il tool verifica_documento_fic', () => {
  ...
  it('un documento pieno di difetti: ogni rilievo col suo dettaglio', async () => {
    ...
    const out = await verifica({ id: 77, tipo: 'autofattura' })
    // La regola 2 passa: questo documento NON ha i DatiFattureCollegate, ed e
    // giusto cosi — passarli via API rompe l XML.
    expect(out.riepilogo).toEqual({ a_norma: 1, rilievi: 5, non_verificati: 0 })
